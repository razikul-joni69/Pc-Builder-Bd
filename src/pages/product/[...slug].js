...
const ProductDetails = ({ data: product }) => {
    return (
        <section className="text-gray-700 body-font overflow-hidden bg-white pt-10">
                ...
                <div className="flex flex-wrap mb-24 -mx-4">
                    <div className="w-full px-4 mb-8 md:w-1/2 md:mb-0">
                        <div className="sticky top-0 overflow-hidden ">
                            <div className="relative mb-6 lg:mb-10 lg:h-[30rem]">
                                ...
                                <img className="object-fill w-full lg:h-full" src={product?.img} alt="" />
                            ...
                            <div className="flex-wrap hidden -mx-2 md:flex">
                                <div className="w-1/2 p-2 sm:w-1/4">
                                    <a className="block border border-gray-200 hover:border-blue-400 dark:border-gray-700 dark:hover:border-blue-300" href="#">
                                        <img className="object-contain w-full lg:h-28" src={product?.img} alt="" />
                                    </a>
                                </div>
                                <div className="w-1/2 p-2 sm:w-1/4">
                                    <a className="block border border-gray-200 hover:border-blue-400 dark:border-gray-700 dark:hover:border-blue-300" href="#">
                    ...
                    <div className="w-full px-4 md:w-1/2">
                        <div className="lg:pl-20">
                            <div className="mb-6 ">
                                <p className="badge badge-info text-white text-xl">{product?.category?.toUpperCase()}</p><br />
                                <p className="badge badge-success text-white text-xl">{product?.status}</p>
                                <h2 className="max-w-xl my-2 text-xl font-semibold leading-loose tracking-wide text-gray-700 md:text-2xl dark:text-gray-300">
                                    {product?.name}
                                </h2>
                                ...
                                <p className="inline-block text-2xl font-semibold text-gray-700 dark:text-gray-400 ">
                                    ...
                                    <span className="ml-3 text-base font-normal text-gray-500 line-through dark:text-gray-400">${parseFloat(product?.price) + 25}</span>
                                </p>
                            </div>
                            <div className="mb-6">
            ...
            <Reviews reviews={product?.reviews} rating={product?.rating} total_ratings={product?.total_ratings} />
        </section>
    );
};
...
export const getStaticProps = async ({ params }) => {
    console.log("paramsProps", params);
...
export const getStaticPaths = async (ctx) => {
    ...
    const paths = categories.flatMap((currentValue) => {
        const test = data?.data[currentValue]?.map((product) => {
            return {
                params: {
                    slug: [product.category, product.name, product?._id], // Ensure slug is an array
                },
            };
        });
        return test || [];
